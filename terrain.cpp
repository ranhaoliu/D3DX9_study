				 ...
				 float heightScale)
{
	...
	//每一行的顶点数
	_numVertsPerRow = numVertsPerRow; //64
	//每一列的顶点数
	_numVertsPerCol = numVertsPerCol; //64
	//每隔格子的大小
	_cellSpacing    = cellSpacing; //10
	//每一行的格子数
	...
	// load heightmap,readRawFile 函数把高度图文件保存的高度值给拷贝到std::vector<int> _heightmap中
	//if( !readRawFile(heightmapFileName) )
	if (!myreadRawFile(heightmapFileName,1024,1024))
	{
		::MessageBox(0, "readRawFile - FAILED", 0, 0);
		::PostQuitMessage(0);
	}

	/*std::ofstream outfile1("test1.txt", std::ios_base::binary);
	for (int i = 0; i < _heightmap.size(); ++i) {
		outfile1 << _heightmap[i];
	}
	outfile1.close();
	...
	// scale heights  
	for(int i = 0; i < _heightmap.size(); i++)
		_heightmap[i] *= heightScale; //heightScale 传过来的 是 0.5

	/*std::ofstream outfile2("test2.txt", std::ios_base::binary);
	for (int i = 0; i < _heightmap.size(); ++i) {
		outfile2 << _heightmap[i];
	}
	outfile2.close();*/
...
bool Terrain::readRawFile(std::string fileName)
{
	...
	std::vector<BYTE> in(_numVertices * 16);
...
bool Terrain::myreadRawFile(std::string fileName, int imgWidth, int imgHeight)
{
	std::vector<BYTE> in(imgWidth * imgHeight);
	std::ifstream inFile(fileName.c_str(), std::ios_base::binary);
	if (!inFile) {
		return false;
	}
	...
	_heightmap.resize(_numVertices);
	for (int index = 0; index < _heightmap.size(); ++index) {
		int i = index / _numVertsPerRow;//第几行，把一位数组分成了 行 列，
		int j = index % _numVertsPerRow;//第几列

		////直接邻域取整
		/*那么如何在原图像中找这个目标点呢，算法公式如下
		src_x = dst_x * (src_width / dst_width);
		src_y = dst_y * (src_height / dst_height);
		现在我们的源宽度是 imgWidth ，目标宽度是 _numVersPerRow
		*/
		//int I = i * imgWidth / _numVertsPerRow;//应该采集原图上哪一行的点
		//int J = j * imgHeight / _numVertsPerCol;//应该采集原图上的那一列的点
		//_heightmap[index] = in[I * imgWidth + J];//采集原图上第几行第几列的像素点
		//end

		//双线性插值
		float I = i * imgWidth / _numVertsPerRow;
		float J = j * imgHeight / _numVertsPerCol;
		float di = I - ::floorf(I);
		float dj = J - ::floorf(J);
		...
		float n0 = d3d::Lerp(in[x0 * imgWidth + y0], in[(x0 + 1) * imgWidth + y0], di);
		float n1 = d3d::Lerp(in[x0 * imgWidth + y0 + 1], in[(x0 + 1) * imgWidth + y0 + 1], di);
		float p = d3d::Lerp(n0, n1, dj);

		_heightmap[index] = ::round(p);
		//end
	}
...
float Terrain::getHeight(float x, float z)
{
	...
	float dx = x - col;//把dx变成0到1之间的小数  
	float dz = z - row;//把dy变成0到1之间的小数
